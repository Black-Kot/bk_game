...
function cart:calc_rail_direction(pos, vel)
	...
	if cart_func:is_int(p.x) and cart_func:is_int(p.z) then
		...
		local pos1 = cart_func.v3:round(p)
		local a = 1;
		if minetest.get_node(pos1).name == "carts:brakerail" then 
			self.velocity = {
			x = 0,
			y = 0,
			z = 0,
...
function cart:on_step(dtime)
	...
	if old_dir.x ~= 0 then
		...
		while true do
			...
			if not cart_func.v3:equal(cart_func:velocity_to_dir(self.old_velocity), dir) then
				...
				pos = self.old_pos
				self.object:moveto(self.old_pos)
				break
			end
		end
	...
	-- Accelerate or decelerate the cart according to the pitch and acceleration of the rail node
		self.velocity = {
			x = self.velocity.x + cart_func:get_sign(self.velocity.x),
			y = self.velocity.y + cart_func:get_sign(self.velocity.y),
			z = self.velocity.z + cart_func:get_sign(self.velocity.z),
		}
			
		-- Place the cart exactly on top of the rail
		if cart_func:is_rail(cart_func.v3:round(pos)) then 
			self.object:moveto({x=pos.x, y=math.floor(pos.y+0.5), z=pos.z})
